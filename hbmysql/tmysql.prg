...
CREATE CLASS TMySQLServer
   ...
   METHOD New( cServer, cUser, cPassword, nPort, nFlags, cCharset )  // Opens connection to a server, returns a server object
   ...
   METHOD sql_set_character_set(cCharset)

ENDCLASS


METHOD New( cServer, cUser, cPassword, nPort, nFlags, cCharset ) CLASS TMySQLServer
   ...
   IF Empty( ::nSocket )
      ::lError := .T.
   ELSEIF !Empty( cCharset )
      mysql_set_character_set(::nSocket, cCharset)
   ENDIF
...
METHOD sql_set_character_set(cCharset) CLASS TMySQLServer
   RETURN mysql_set_character_set(::nSocket, cCharset)
